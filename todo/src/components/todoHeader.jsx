...
export function TodoHeader({ todoHeader, todoData }) {
    ...
    if (isEditing) {
        ...
        if (toBeCompletedAt && toBeCompletedAt < Date.now() && status !== 'completed') { alertContent = <AlertCircle className="todo-alert"/>};

        todoContent = (<div className="todo-text-wrapper">
                            <span className={`todo-text ${status === "active" ? "todo-active" : "todo-done"}`}>
                                {text}
                                {alertContent}
                            </span>
                       </div>);
